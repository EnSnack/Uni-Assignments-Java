...
public class Turtle extends Actor {
    ...
    public void kochCurve(int n, double size) {
        if(n>=1) {
          kochCurve(n-1, size/2); turn(-60);
          kochCurve(n-1, size/2); turn(120);
          kochCurve(n-1, size/2); turn(-60);
          kochCurve(n-1, size/2);
        } else {
            move(size/2);
        }
    }
    ...
    public void sierpinskiCurve(int n, double size) {
        if(n>=1) {
            sierpinskiCurve(n-1, size/2); penUp(); move(size/2); penDown();
            sierpinskiCurve(n-1, size/2); penUp(); move(size/2); turn(120); move(size/2); penDown();
            sierpinskiCurve(n-1, size/2); penUp(); move(size/2); turn(120); move(size); turn(120); penDown();
        } else {
            triangle(size);
        }
    }
    ...
    public void squarePatternDiagonal(int n, double size) {
        if(n>=2) {
            squarePatternDiagonal(n-1, size/2); penUp(); jump(size/2, 0); penDown();
            squarePatternDiagonal(1, size/2); penUp(); jump(-size/2, size/2); penDown();
            squarePatternDiagonal(1, size/2); penUp(); jump(size/2, 0); penDown();
            squarePatternDiagonal(n-1, size/2); penUp(); jump(-size/2, -size/2); penDown();          
        } else {
            polygon(4, size);
        }
    }
    ...
    public void squarePatternFour(int n, double size) {
        if(n>=2) {
            squarePatternFour(n-1, size/3); penUp(); jump(size/3, 0); penDown();
            squarePatternFour(n-1, size/3); penUp(); jump(-size/3, -size/3); penDown();
            squarePatternFour(n-1, size/3); penUp(); jump(size/3, 0); penDown();
