...
public class GUI {
    ...
    private int CTRL = Event.CTRL_MASK,
                CTRL_SHIFT = CTRL | Event.SHIFT_MASK;
    ...
    public GUI(){
        ...
        timer = new Timer(frameDelay, e->{
            ...
            try{
                ...
                //This error happens in the very rare case that the player arrives at
                //a CapitalCity in a MafiaCountry and is robbed for more than,
                //
                //             rob > p.getMoney()+bonus-toll             (*)
                //
                //In this case, nextInt in CapitalCity gets <= 0 as argument and throws
                //an IllegalArgumentException. For simplicity, we just ignore this, but
                //a fun little extra assignment could be to fix this error. :-)
                //                                                         -Nikolaj
    ...
    /**
     * Creates the menu bar and each of its options.
     */
    private void createMenuBar(JMenuBar menu) {
        ...
        /** PAUSE/RESUME GAME */
        JMenuItem pauseGameItem = new JMenuItem("Pause/Resume game");
        gameMenu.add(pauseGameItem);
        pauseGameItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P, CTRL_SHIFT));
        pauseGameItem.addActionListener(e -> pauseResumeButton.doClick());
        ...
        //slow
        JMenuItem slowSpeed = new JMenuItem("Slow");
        speedMenu.add(slowSpeed);
        slowSpeed.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_1, CTRL));
        slowSpeed.addActionListener(e -> setSpeed(1));
        //medium
        JMenuItem mediumSpeed = new JMenuItem("Medium");
        speedMenu.add(mediumSpeed);
        mediumSpeed.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_2, CTRL));
        mediumSpeed.addActionListener(e -> setSpeed(2));
        //medium
        JMenuItem fastSpeed = new JMenuItem("Fast");
        speedMenu.add(fastSpeed);
        fastSpeed.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_3, CTRL));
        fastSpeed.addActionListener(e -> setSpeed(3));
        //sonic
        JMenuItem sonicSpeed = new JMenuItem("Sonic");
        speedMenu.add(sonicSpeed);
        sonicSpeed.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_4, CTRL));
        sonicSpeed.addActionListener(e -> setSpeed(4));
    ...
    private void applyOptions() {
        ...
        try{
            ...
            if(tollSize < 0 || riskRob < 0 || tollSize > 50 || riskRob > 50){
                builder.append("'Toll size' and 'Risk rob' must be between 0 and 50.\n");
            }
        } catch (NumberFormatException e){
            builder.append("'Toll size' and 'Risk rob' must be integers.\n");
        }
        
        //Min loss and max loss
        ...
        try{
            ...
            if(minLoss < 0 || maxLoss < 0 || minLoss > 100 || maxLoss > 100){
                builder.append("'Minimum loss' and 'Maximum loss' must be between 0 and 100.\n");
            }
            if(minLoss > maxLoss){
                builder.append("'Minimum loss' must be smaller than or equal to 'Maximum loss'.\n");
            }
        } catch (NumberFormatException e){
            builder.append("'Minimum loss' and 'Maximum loss' must be integers.\n");
        }
        if(builder.length()>0) {
            JOptionPane.showMessageDialog(mainFrame, builder.toString(), "Malformed input", JOptionPane.ERROR_MESSAGE);
        } else {
    ...
    public JFrame createOptionsDialogBox(){
        ...
        JLabel minLossVal = new JLabel("€ in [0,100]");
        tollAndRobberyPanel.add(minLossVal);
        ...
        JLabel maxLossVal = new JLabel("€ in [0,100]");
        tollAndRobberyPanel.add(maxLossVal);
    ...
    /**
     * Saves the log of the current game.
     */
    private void testSaveButton() {
        int returnVal = fileChooser.showSaveDialog(null);
        if(returnVal == JFileChooser.APPROVE_OPTION) {
            try {
                ObjectOutputStream os = new ObjectOutputStream(new FileOutputStream(fileChooser.getSelectedFile().getAbsolutePath()));
                os.writeObject((Log) game.getLog());
                os.close();
            }
            catch(FileNotFoundException e) {
                JOptionPane.showMessageDialog(mainFrame, "File cannot be saved in this location.", "File Not Found", JOptionPane.ERROR_MESSAGE);
                return;
            }
            catch(InterruptedIOException e) {
                JOptionPane.showMessageDialog(mainFrame, "Saving was interrupted, please try again.", "Save Interruption", JOptionPane.ERROR_MESSAGE);
                return;
            }
            catch(StreamCorruptedException e) {
                JOptionPane.showMessageDialog(mainFrame, "The save was corrupted, please try again.", "Save Corrupted", JOptionPane.ERROR_MESSAGE);
                return;
            }
            catch(IOException e) {
                JOptionPane.showMessageDialog(mainFrame, "An error was found, please try again.", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
    }
    ...
    /**
     * Plays the log at the given path.
     */
    private void testPlayButton(){
        int returnVal = fileChooser.showOpenDialog(null);
        if(returnVal == JFileChooser.APPROVE_OPTION) {
            try {
                ObjectInputStream os = new ObjectInputStream(new FileInputStream(fileChooser.getSelectedFile().getAbsolutePath()));
                Log log = (Log) os.readObject();
                os.close();
                game.playLog(log);
            }
            catch(FileNotFoundException e) {
                JOptionPane.showMessageDialog(mainFrame, "File cannot be found, please try again.", "File Not Found", JOptionPane.ERROR_MESSAGE);
                return;
            }
            catch(InterruptedIOException e) {
                JOptionPane.showMessageDialog(mainFrame, "Loading was interrupted, please try again.", "Save Interruption", JOptionPane.ERROR_MESSAGE);
                return;
            }
            catch(StreamCorruptedException e) {
                JOptionPane.showMessageDialog(mainFrame, "This file is corrupted.", "Save Corrupted", JOptionPane.ERROR_MESSAGE);
                return;
            }
            catch(ClassNotFoundException e) {
                JOptionPane.showMessageDialog(mainFrame, "Class was not found, please try again.", "Class Not Found", JOptionPane.ERROR_MESSAGE);
                return;
            }
            catch(IOException e) {
                JOptionPane.showMessageDialog(mainFrame, "An error was found, please try again.", "Error", JOptionPane.ERROR_MESSAGE);
...
class WorldPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for(Player p : players){
            ...
            g2d.drawString(p.getMoney()+" â‚¬", 150, y+10);
            //Draw bar
        ...
        if(GUI.hover != null){
            ...
            g2d.setFont(FONT_BODY);
            g2d.drawString(GUI.hover.getValue()+" â‚¬", 350, 486);
            g2d.setFont(FONT_HEADER);
