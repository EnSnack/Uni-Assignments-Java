...
public class ClosestBall {
    public int computeClosest(ArrayList<Integer> players, ArrayList<Integer> balls) {
        ...
        int s = 0, e = 0, cP = 0, cB = 0, diff = 9999;
        while(s < players.size() && e < balls.size()) {
            if(Math.abs(players.get(s) - balls.get(e)) < diff) {
                diff = Math.abs(players.get(s) - balls.get(e));
                cP = s;
                cB = e;
            }
            
            if(players.get(s) <= balls.get(e)) {
                s++;
            } else {
                e++;
            }
        }
        return Math.abs(players.get(cP) - balls.get(cB));
    }
